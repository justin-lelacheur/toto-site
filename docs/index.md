---
layout: home
title: toto foundry - The Enterprise Operating System
is_homepage: true
---

<main class="home-product">
  <section class="home-hero">
    <div class="home-hero__inner">
      <div class="home-hero__copy">
        <p class="home-eyebrow">AI-native enterprise operating system</p>
        <h1>toto foundry.<br>Enterprise, ready for change.</h1>
        <p class="home-hero__lead">The AI-native enterprise operating system from toto, starting with CRM.</p>
        <div class="home-actions">
          <a class="home-action home-action--light" href="/crm-module/">Explore CRM</a>
          <a class="home-text-link" href="/platform-overview/">Explore toto foundry <span aria-hidden="true">&rarr;</span></a>
        </div>
      </div>
      <div class="home-hero__signal" aria-label="toto operating model: customer, workflow, and execution">
        <div class="home-signal__header">
          <span>Live operating context</span>
        </div>
        <div class="home-signal__diagram" aria-hidden="true">
          <div class="home-signal__node home-signal__node--customer">
            <span class="home-signal__node-dot"></span>
            <strong>Customer</strong>
            <small>relationships + consent</small>
          </div>
          <div class="home-signal__path home-signal__path--one"><span></span></div>
          <div class="home-signal__node home-signal__node--workflow">
            <span class="home-signal__node-dot"></span>
            <strong>Workflow</strong>
            <small>policy + approval</small>
          </div>
          <div class="home-signal__path home-signal__path--two"><span></span></div>
          <div class="home-signal__node home-signal__node--execution">
            <span class="home-signal__node-dot"></span>
            <strong>Execution</strong>
            <small>approved outcome</small>
          </div>
        </div>
        <div class="home-signal__frame">
          <p>One trusted model</p>
          <span>Meaning stays connected to the work.</span>
        </div>
      </div>
    </div>
  </section>

  <section class="home-section home-section--paper">
    <div class="home-section__inner home-section__split home-intro">
      <div class="home-section__heading">
        <p class="home-eyebrow">One operating model</p>
        <h2>When the business changes,<br>the system should move with it.</h2>
      </div>
      <div class="home-section__content">
        <p class="home-copy">toto foundry connects enterprise meaning to everyday execution. Domain-owned services let the platform evolve one microservice capability at a time, while people, processes, policies, and AI work from the same current picture of the business.</p>
      </div>
    </div>
  </section>

  <section class="home-section home-section--signal">
    <div class="home-section__inner home-section__split">
      <div class="home-section__heading">
        <p class="home-eyebrow">Designed for consequential work</p>
        <h2>Clarity at<br>every step.</h2>
      </div>
      <div class="home-principles home-section__content">
        <article>
          <h3>A standardized customer model</h3>
          <p>The common data model is grounded in the enterprise ontology, connecting accounts, contacts, consent, cases, and ownership so every workflow and agent shares the same customer context.</p>
        </article>
        <article>
          <h3>Owners and agents evolve work</h3>
          <p>In toto foundry Studio, business function owners collaborate with AI agents to continuously refine features and processes as the business evolves.</p>
        </article>
        <article>
          <h3>One clear job per agent</h3>
          <p>Agents handle defined tasks within set functional boundaries—for example, a validation agent checks proposed changes instead of trying to do everything.</p>
        </article>
        <article>
          <h3>Change without a rebuild</h3>
          <p>Evolve domain models, workflows, and policies incrementally, so new capabilities build on existing operations rather than forcing a platform-wide replacement.</p>
        </article>
      </div>
    </div>
  </section>

  <section class="home-section home-section--open">
    <div class="home-section__inner home-section__split home-open">
      <div class="home-section__heading">
        <p class="home-eyebrow">100% open</p>
        <h2>Open source.<br>Forever.</h2>
      </div>
      <div class="home-open__copy home-section__content">
        <p>toto commits toto foundry to a free and open-source core, so customers can inspect the foundation, avoid opaque lock-in, and retain a meaningful path to operate and extend it themselves.</p>
        <p>The core is built with platform-defining open-source programs: Apache Kafka for events, Conductor OSS for durable workflows, Neo4j and Protégé for ontology work, LangGraph for bounded AI, and FastAPI, Keycloak, OpenFGA, and Open Policy Agent for the integration and control plane.</p>
        <div class="home-open__examples" aria-label="Open-source foundations">
          <code>Apache Kafka</code><code>Conductor OSS</code><code>Neo4j</code><code>Protégé</code><code>LangGraph</code><code>FastAPI</code><code>Keycloak</code><code>OpenFGA</code><code>Open Policy Agent</code>
        </div>
      </div>
    </div>
  </section>

  <section class="home-section home-section--crm">
    <div class="home-section__inner home-section__split home-crm">
      <div class="home-section__heading">
        <p class="home-eyebrow">First domain module</p>
        <h2>CRM is where<br>toto foundry begins.</h2>
      </div>
      <div class="home-crm__copy home-section__content">
        <p>Customer 360, account and contact relationships, consent-aware engagement, cases, tasks, and guided workflows in the first toto foundry module.</p>
        <a class="home-text-link" href="/crm-module/">Explore the CRM module <span aria-hidden="true">&rarr;</span></a>
      </div>
    </div>
  </section>

  <section class="home-section home-section--paper">
    <div class="home-section__inner home-section__split home-process">
      <div class="home-section__heading home-section__heading--wide">
        <p class="home-eyebrow">From intent to outcome</p>
        <h2>Business meaning in.<br>Governed action out.</h2>
      </div>
      <ol class="home-flow home-flow--cards home-section__content">
        <li class="home-flow__card">
          <strong>Model</strong>
          <div class="home-flow__tokens" aria-label="Customer model fields">
            <code>party_id</code><code>account_owner</code><code>consent_status</code>
          </div>
          <p>Define the customer, organization, policy, and relationship that make the work meaningful.</p>
        </li>
        <li class="home-flow__card">
          <strong>Design</strong>
          <div class="home-flow__tokens" aria-label="Workflow example">
            <code>onboard</code><code>review</code><code>activate</code>
          </div>
          <p>Shape forms, workflows, approvals, and exceptions in a shared studio.</p>
        </li>
        <li class="home-flow__card">
          <strong>Operate</strong>
          <div class="home-flow__tokens" aria-label="Runtime signals">
            <code>customer.updated</code><code>workflow.active</code><code>case.escalated</code>
          </div>
          <p>Run the work through durable processes, versioned events, and owned domain services.</p>
        </li>
        <li class="home-flow__card">
          <strong>Improve</strong>
          <div class="home-flow__tokens" aria-label="Exception examples">
            <code>duplicate_match</code><code>consent_missing</code><code>owner_reassigned</code>
          </div>
          <p>Use evidence from real execution to refine the next change with confidence.</p>
        </li>
      </ol>
    </div>
  </section>

  <section class="home-section home-section--close">
    <div class="home-section__inner home-section__split home-close">
      <div class="home-section__heading">
        <p class="home-eyebrow">Built to evolve</p>
        <h2>Modernize the operation.<br>Keep control of the enterprise.</h2>
      </div>
      <div class="home-close__copy home-section__content">
        <p>Bring existing systems forward in stages, preserve the decisions that matter, and make the next capability easier to build than the last.</p>
        <div class="home-actions">
          <a class="home-action home-action--dark" href="/why-toto/">Why toto <span aria-hidden="true">&rarr;</span></a>
          <a class="home-text-link" href="/contact/">Start a conversation <span aria-hidden="true">&rarr;</span></a>
        </div>
      </div>
    </div>
  </section>
</main>
